...
def list_tasks(db: Session, user_id: int, *, status=None, priority=None, q=None,
               due_from: Optional[date]=None, due_to: Optional[date]=None,
               page:int=1, page_size:int=20) -> Tuple[List[TaskORM], int]:
    stmt = select(TaskORM).where(TaskORM.user_id==user_id)
    if status:   stmt = stmt.where(TaskORM.status==status)
    if priority: stmt = stmt.where(TaskORM.priority==priority)
    if q:        stmt = stmt.where(func.lower(TaskORM.title).like(f"%{q.lower()}%"))
    if due_from: stmt = stmt.where(TaskORM.due_date >= due_from)
    if due_to:   stmt = stmt.where(TaskORM.due_date <= due_to)
    total = db.execute(stmt.with_only_columns(func.count())).scalar_one()
    items = db.execute(stmt.order_by(TaskORM.created_at.desc())
